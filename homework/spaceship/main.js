...
var isVarType = function() {
    ...
    for (var i in types) {
        var type = types[i];
        (function(t) {
            isVarType["is" + t] = function(data) {
                return !(!data) && Object.prototype.toString.call(data) === "[object " + t + "]";
            }
        })(type)
...
SimpleDialog.prototype = {
    init: function(){
        ...
        $dialog.addEventListener("click", function(e){
            ...
            var {hasCancel, cancelCallback, hasSure, sureCallback} = that.options;
    ...
    getInnerDialogNode: function() {
        ...
        var {hasTitle, title, type, html, hasClose, hasCancel, cancelBtn, hasSure, sureBtn} = this.options;

        var renderOpBtnHTML = function(){
            var sureBtnHTML = `<button class='sure'>${sureBtn}</button>`,
        ...
        var renderCloseHTML = function() {
            return hasClose ? `<span class="close"><i>|</i><i>|</i></span>` : "";
        }
       
        var HTML = `<p class=${hasTitle && "title"}>${hasTitle ? title : ""}</p>
...
var USERSPACECOORD = null;
var ROBOTSPACECOORDLIST = [];
...
Cell.prototype = {
    init: function() {
        ...
        var $ipt = $cell.getElementsByTagName("input")[0];
...
var initStatus = function() {
    if (INITIDX == 0) {
        var className = $table.className.replace('show','') + " hide";
